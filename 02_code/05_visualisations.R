...
indicators_graph <- required_indicators %>%
  ...
  mutate(indicator = recode(indicator, weighted_ANC4 = "ANC4+", weighted_SBA = "SBA")) %>%
...
required_indicators_table <- required_indicators %>%
  ...
  tab_header(
    title = md(
      "**Population-Weighted Coverage of Maternal Health Indicators by U5MR Status**"
    ),
    subtitle = md(
      "*ANC4+ and SBA coverage estimates weighted by projected births in 2022*"
    )
  ) %>%
  cols_label(
  ...
  fmt_percent(columns = c(weighted_ANC4, weighted_SBA),
              scale_values = FALSE) %>%
  cols_align(align = "center", columns = everything()) %>%
  tab_source_note(source_note = "Data Source: UNICEF Global Data Repository, UN World Population Prospects 2022") %>%
  opt_table_font(font = list(google_font(name = "Open Sans"), default_fonts()))
...
gtsave(
  ...
  filename = here::here(
    "03_figures",
    "indicators_coverage_by_u5mr_status_table.png"
  )
)
1
