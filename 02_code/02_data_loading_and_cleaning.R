...
on_track_countries <- read_xlsx("01_data/raw_data/On-track and off-track countries.xlsx") %>%
  ...
  set_variable_labels(iso3code = "Country Code",
...
indicator_estimates <- read_xlsx("01_data/raw_data/GLOBAL_DATAFLOW_2018-2022.xlsx") %>%
  ...
  rename(official_name = geographic_area) %>%
  ## Convert Variables to factors
  mutate(official_name = as_factor(official_name),
         indicator = as_factor(indicator))

## 3. Join the on_track_countries with the indicator estimates
indicators_on_track <- on_track_countries %>% left_join(indicator_estimates, by = "official_name") %>%
  ...
  pivot_wider(names_from = indicator, values_from = latest_value)

#### 4. Import the weights data _ projected births for 2020

weights_data <- read_xlsx(
  "01_data/raw_data/WPP2022_GEN_F01_DEMOGRAPHIC_INDICATORS_COMPACT_REV1.xlsx",
  sheet = "Projections"
) %>%
  ## Clean the variable names using `janitor`
  clean_names() %>%
  # Select only the relevant columns
  select(region_subregion_country_or_area, births_thousands, year) %>%
  ## Rename the region_subregion_country_or_area variable to official_name for merging with other data sets
  rename(official_name = region_subregion_country_or_area) %>%
...
complete_merged_data <- indicators_on_track %>%
  left_join(weights_data, by = c("official_name")) %>%
  filter(
...
write_xlsx(complete_merged_data,
           "01_data/processed_data/complete_merged_data.xlsx")
